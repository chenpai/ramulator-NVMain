...
class Cache{
public:
  double cache_read_miss;
  double cache_write_miss;
  double cache_total_miss;
  double cache_eviction;
  double cache_read_access;
  double cache_write_access;
  double cache_total_access;
  double cache_mshr_hit;
  double cache_mshr_unavailable;
  double cache_set_unavailable;
  Stats *stats; //2.0
  std::string statName;
public:
  Stats* GetStats( );//2.0
  ...
  struct Line {
	...
	Line(long addr, long tag, int coreid):
        addr(addr), tag(tag), lock(true), dirty(false), coreid(coreid){}
    Line(long addr, long tag, bool lock, bool dirty):
        addr(addr), tag(tag), lock(lock), dirty(dirty) {}
	Line(long addr, long tag, bool lock, bool dirty, bool access,int coreid):
        addr(addr), tag(tag), lock(lock), dirty(dirty), access(access), coreid(coreid){}
	Line(long addr, long tag, bool lock, bool dirty, int coreid):
        addr(addr), tag(tag), lock(lock), dirty(dirty), coreid(coreid){}	
  };
  ...
  int latency[int(Level::MAX)] = {2, 2 + 6, 2 + 6 + 15};
  int latency_each[int(Level::MAX)] = {2, 6, 15};
  ...
  bool send(NVMainRequest req,bool& mshr);

  void concatlower(Cache* lower);

  void callback(NVMainRequest& req);
...
class CacheSystem {
public:
  CacheSystem(Config& configs, std::function<bool(NVMainRequest,bool&)> send_memory,
			  NVMObject_hook * getChild):
    send_memory(send_memory),getChild(getChild) {
  ...
  std::list<std::pair<long, NVMainRequest> > wait_list;
  ...
  std::list<std::pair<long, NVMainRequest> > hit_list;

  std::function<bool(NVMainRequest,bool&)> send_memory;
  NVMObject_hook * getChild;
  long clk = 0;
  int tick();
